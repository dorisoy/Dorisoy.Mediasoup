...
public unsafe class H264Decoder : IVideoDecoder
{
    ...
    private void Initialize()
    {
        try
        {
            ...
            _initialized = true;
            _logger.LogInformation("H264 decoder initialized successfully");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to initialize H264 decoder");
        }
    }
    ...
    /// 解码 H264 帧
    /// </summary>
    /// <param name="frameData">H264 编码数据（NAL 单元）</param>
    ...
    public bool Decode(byte[] frameData)
    {
        ...
        lock (_lock)
        {
            ...
            {
                _logger.LogError(ex, "Error decoding H264 frame");
                return false;
            }
        }
    }
