...
public class TitleBar : System.Windows.Controls.Control, IThemeControl
{
    ...
    /// <summary>
    /// Property for <see cref="MenuContent"/>.
    /// </summary>
    ...
    /// <summary>
    /// Property for <see cref="RightMenuContent"/>.
    /// </summary>
    public static readonly DependencyProperty RightMenuContentProperty = DependencyProperty.Register(
        nameof(RightMenuContent),
        typeof(object),
        typeof(TitleBar),
        new PropertyMetadata(null)
    );
    ...
    /// <summary>
    /// Gets or sets the menu content displayed after the header in the <see cref="TitleBar"/>.
    /// This is typically used for application menus.
    /// </summary>
    ...
    /// <summary>
    /// Gets or sets the right menu content displayed before the window control buttons in the <see cref="TitleBar"/>.
    /// This is typically used for user menu or additional controls.
    /// </summary>
    ...
    private IntPtr HwndSourceHook(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam, ref bool handled)
    {
        ...
        if (message == User32.WM.NCHITTEST && (TrailingContent is UIElement || Header is UIElement || MenuContent is UIElement || RightMenuContent is UIElement))
        {
            UIElement? headerLeftUIElement = Header as UIElement;
            UIElement? menuContentUIElement = MenuContent as UIElement;
            UIElement? headerRightUiElement = TrailingContent as UIElement;
            UIElement? rightMenuContentUIElement = RightMenuContent as UIElement;
            ...
            if (headerLeftUIElement is not null && headerLeftUIElement != _titleBlock)
            {
                isMouseOverHeaderContent =
                    ...
                    || (menuContentUIElement?.IsMouseOverElement(lParam) ?? false)
                    || (headerRightUiElement?.IsMouseOverElement(lParam) ?? false)
                    || (rightMenuContentUIElement?.IsMouseOverElement(lParam) ?? false);
            }
            else
            {
                isMouseOverHeaderContent = 
